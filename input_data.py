...
def get_frames_data(filename, num_frames_per_clip=16):
  ...
  for parent, dirnames, filenames in os.walk(filename):
    if(len(filenames)<num_frames_per_clip):
      #print('Warning: number of frames is less than num_frames_per_clip (%d < %d)'
        #% (len(filenames), num_frames_per_clip))
      return [], s_index
...
def read_clip_and_label(filename, batch_size, start_pos=-1, num_frames_per_clip=16, crop_size=112, shuffle=False):
  ...
  next_batch_start = -1
  lines = list(lines)
  #print('Info: len of video is', len(lines))
  ...
  for index in video_indices:
    ...
    tmp_label = line[1]
    #if not shuffle:
      #print("Info: Loading a video clip from {}...".format(dirname))
